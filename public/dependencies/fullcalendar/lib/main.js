...
var FullCalendar = (function (exports) {
		...
		for (var n; (i = u.length); )
			(n = u.sort(function (n, l) {
				...
				n.some(function (n) {
					...
					n.__d &&
						((o = (r = (l = n).__v).__e),
						(f = l.__P) && ((u = []), ((i = a({}, r)).__v = i), (t = A(f, r, i, l.__n, void 0 !== f.ownerSVGElement, null, u, null == o ? w(r) : o)), T(u, r), t != o && k(r)));
				});
	}
		...
		__e: function (n, l) {
			for (var u, i; (l = l.__); )
				if ((u = l.__c) && !u.__)
					try {
						if (
							(u.constructor && null != u.constructor.getDerivedStateFromError && ((i = !0), u.setState(u.constructor.getDerivedStateFromError(n))),
    ...
    */
	...
	function parse(str) {
		...
		if (m) {
			var marker = new Date(
				Date.UTC(Number(m[1]), m[3] ? Number(m[3]) - 1 : 0, Number(m[5] || 1), Number(m[7] || 0), Number(m[8] || 0), Number(m[10] || 0), m[12] ? Number('0.' + m[12]) * 1000 : 0)
			);
	...
	var RenderHook = /** @class */ (function (_super) {
		...
		RenderHook.prototype.render = function () {
			...
			return createElement(MountHook, { hookProps: hookProps, didMount: props.didMount, willUnmount: props.willUnmount, elRef: this.handleRootEl }, function (rootElRef) {
				return createElement(ContentHook, { hookProps: hookProps, content: props.content, defaultContent: props.defaultContent, backupElRef: _this.rootElRef }, function (
					innerElRef,
					innerContent
				) {
			...
			return createElement(ViewContextType.Consumer, null, function (context) {
				return createElement(ViewRoot, { viewSpec: context.viewSpec }, function (rootElRef, viewClassNames) {
					...
					return createElement(
						RenderHook,
						{ hookProps: hookProps, classNames: options.classNames, content: options.content, didMount: options.didMount, willUnmount: options.willUnmount, elRef: rootElRef },
			...
			return createElement(
				...
				function (rootElRef, customClassNames, innerElRef, innerContent) {
					return createElement(
						'th',
						__assign(
							{ ref: rootElRef, className: classNames.concat(customClassNames).join(' '), 'data-date': !dayMeta.isDisabled ? formatDayString(date) : undefined, colSpan: props.colSpan },
    ...
    */
	...
	function refineHookProps(raw) {
		...
		return __assign(
			__assign(__assign({ date: dateEnv.toDate(date), view: raw.viewApi }, dayMeta), { dayNumberText: raw.showDayNumber ? dateEnv.format(date, DAY_NUM_FORMAT) : '' }),
			raw.extraProps
		);
	}
		...
		function Calendar(el, optionOverrides) {
			...
			_this.handleRenderRequest = function () {
				if (_this.isRendering) {
					...
					render(
						createElement(CalendarRoot, { options: currentData_1.calendarOptions, theme: currentData_1.theme, emitter: currentData_1.emitter }, function (
							classNames,
							height,
							isHeightAuto,
							forPrint
    ...
    */
									...
									createElement(
										...
										createElement(
											...
											function (rootElRef, classNames, innerElRef, innerContent) {
												return createElement(
													'a',
													{ onClick: _this.handleMoreLinkClick, ref: rootElRef, className: ['fc-daygrid-more-link'].concat(classNames).join(' ') },
													innerContent
												);
											}
										)
	...
	function hasListItemDisplay(seg) {
		...
		return (
			display === 'list-item' || (display === 'auto' && !seg.eventRange.def.allDay && seg.firstCol === seg.lastCol) // can't be multi-day
		);
	}
					...
					props.segs.map(function (seg) {
						...
						return createElement(
							...
							hasListItemDisplay(seg)
								? createElement(
										TableListItemEvent,
										__assign({ seg: seg, isDragging: false, isSelected: instanceId === selectedInstanceId, defaultDisplayEventEnd: false }, getSegMeta(seg, todayRange))
								  )
								: createElement(
										TableBlockEvent,
										__assign(
											{ seg: seg, isDragging: false, isResizing: false, isDateSelecting: false, isSelected: instanceId === selectedInstanceId, defaultDisplayEventEnd: false },
											getSegMeta(seg, todayRange)
										)
								  )
						);
    ...
    */
				...
				chunks: [
					{
						...
						content: function (arg) {
							...
							return createElement(
								...
								createElement('table', null, arg.tableColGroupNode, createElement('tbody', null, createElement(TimeBodyAxis, { slatMetas: slatMetas }))),
								createElement(
									'div',
	...
	var TimeCol = /** @class */ (function (_super) {
		...
		TimeCol.prototype.render = function () {
			...
			return createElement(DayCellRoot, { elRef: props.elRef, date: props.date, dateProfile: props.dateProfile, todayRange: props.todayRange, extraHookProps: props.extraHookProps }, function (
				rootElRef,
				classNames,
				dataAttrs
			) {
		...
		TimeCol.prototype.renderPrintFgSegs = function (segs) {
			...
			return segs.map(function (seg) {
				return createElement(
					...
					createElement(
						TimeColEvent,
						__assign({ seg: seg, isDragging: false, isResizing: false, isDateSelecting: false, isSelected: false, isCondensed: false }, getSegMeta(seg, props.todayRange, props.nowDate))
					)
				);
			});
		};
		TimeCol.prototype.renderPositionedFgSegs = function (segs, segIsInvisible, isDragging, isResizing, isDateSelecting) {
			...
			return segs.map(function (seg) {
				var instanceId = seg.eventRange.instance.instanceId;
				var isMirror = isDragging || isResizing || isDateSelecting;
				var positionCss = isMirror ? __assign({ left: 0, right: 0 }, _this.computeSegTopBottomCss(seg)) : _this.computeFgSegPositionCss(seg);
				return createElement(
					'div',
					{
		...
		TimeCol.prototype.renderNowIndicator = function (segs) {
			...
			return segs.map(function (seg, i) {
				return createElement(NowIndicatorRoot, { isAxis: false, date: date, key: i /* key doesn't matter. will only ever be one */ }, function (
					rootElRef,
					classNames,
					innerElRef,
					innerContent
	...
	var TimeColMisc = /** @class */ (function (_super) {
		...
		TimeColMisc.prototype.render = function () {
			var props = this.props;
			return createElement(DayCellContent, { date: props.date, dateProfile: props.dateProfile, todayRange: props.todayRange, extraHookProps: props.extraHookProps }, function (
				innerElRef,
				innerContent
			) {
